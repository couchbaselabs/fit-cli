...
The AWS region and VPC are fixed (region `us-west-2`, VPC `fit-cli-vpc`) and are not configurable because:
...
This has nothing to do with Private Endpoint (PE) testing specifically — PE testing works from any VPC. The only real constraint is that the EC2 instance and the Capella private endpoint must land in the same VPC's default security group (see `privateEndpointVpcSgId` in environments.json5).
