...
test("setup.capellaEnvironments rejects an endpoint that isn't a URL", () => {
  assert.throws(
    ...
    (err: Error) => err instanceof InvalidDefinitionError && /must be an https origin/.test(err.message),
  );
});
...
test("setup.capellaEnvironments rejects a plaintext http endpoint (credentials are sent to it)", () => {
  assert.throws(
    () =>
      parseDefinition(
        SANDBOX_CAPELLA.replace(
          "https://api.sbx-1234.nonprod-project-avengers.com",
          "http://api.sbx-1234.nonprod-project-avengers.com",
        ),
      ),
    (err: Error) => err instanceof InvalidDefinitionError && /must be an https origin/.test(err.message),
  );
