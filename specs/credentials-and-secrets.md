...
[CAPELLA4] A sandbox environment (`sandbox: true` in environments.json5, currently just `sandbox`) is a pre-deployed control plane, and inverts both of the above.  Its endpoints and org id are not known ahead of time, so the wizard asks for its URL (any of the `ui.`/`api.`/`cloudapi.` hosts — the other two are derived) plus the org id, and the definition file carries them (`setup.capellaEnvironments`); its accounts are recreated along with it, so all four credentials must come from the CAPELLA_USER/CAPELLA_PASS/CAPELLA_API_KEY/CAPELLA_API_SECRET env vars — not the secret, and not a personal `config` account, which a sandbox ignores (a standing-env account there would authenticate against the wrong control plane) — a run that lacks them fails before provisioning anything.  Its secret holds only the internal support and override tokens, which survive a redeploy; it is fetched the same way a standing env's secret is (not best-effort), but only when those tokens aren't already supplied via CAPELLA_INTERNAL_SUPPORT_TOKEN/CAPELLA_OVERRIDE_TOKEN [CONFIG2] — so a run that needs them and can't reach AWS fails fast rather than silently dropping cbcollect support.  Otherwise it behaves exactly like dev.
