...
5m - overall deadline for a freshly launched GCP box to accept an IAP-tunneled SSH connection (`waitForIapSsh`).
60s - `gcloud --version` preflight (`preflightGcloudCli`); it's a local, offline call, so if it hasn't answered by then it never will.
2m - per-call bound on each GCP compute client call (instance insert/get/delete and zone-operation waits). The `@google-cloud/compute` client sets no deadline of its own, so every call is wrapped (`withCallTimeout`) — otherwise a polling loop's own deadline is never re-checked.
5s - grace period a subprocess killed for exceeding its `timeoutMs` gets on SIGTERM before it is SIGKILLed (`proc.ts`).
